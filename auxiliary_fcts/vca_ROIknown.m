function [Ae, indice, Rp] = vca_ROIknown(R,endm_roi,p)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
         ...
         if SNR < SNR_th,   
                ...
                if snr_input==0, % it means that the projection is already computed
                     ...
                     x_p =  Ud' * R_o;                 % project the zeros mean data onto p-subspace
                     t_roi = Ud' * (endm_roi-R_m); % project endmember ROI into the subspace
                    ...
                    lim = 0.1*L;
                    t1 = find(sum(abs(x_p-repmat(t_roi,1,N)),1)<lim);
                    [G,ind] = sort(sum(abs(x_p(:,t1)-repmat(t_roi,1,length(t1))),1));
                    clear G
                    t2 = [];
                    if ~isempty(ind)
...
%               Clear ROI pixel
                ...
                x =  Ud' * R;
                t_roi = Ud' * endm_roi; % project endmember ROI into the subspace
...
%                 t_roi =  t_roi./ repmat( sum( x(:,t2) .* u ) ,[d 1]);
                t_roi =  t_roi./ repmat( sum( t_roi .* u ) ,[d 1]);
...
indice = zeros(1,p-1);
A = zeros(p,p);
A(:,1) = t_roi; %Fix ROI endmember
A(p,2) = 1;

for i=2:p %Begin loop in the next endmember
      w = rand(p,1);   
      f = w - A*pinv(A)*w;
      f = f / sqrt(sum(f.^2));
      ...
      [v_max indice(i-1)] = max(abs(v));
      A(:,i) = y(:,indice(i-1));        % same as x(:,indice(i))
end

Ae = [endm_roi Rp(:,indice)]; %Fix ROI endmember
