function [Y,A0,M0,B,B_out,alpha,E,BP] = generate_parameters_dpa(A,M,V,vect_frame_time,K,M_dpa)
A0 = zeros(size(A));
[temp,ind] = max(A,[],1);
ind(sum(A)==0)=0;
% Clustering
...
vect_parameters_low(1,:) = [1,0.4,0.15,0.04];
vect_parameters_low(2,:) = [1,0.3,0.15,0.13];
vect_parameters_high(1,:) = [1.7,0.4,0.15,0.04];
vect_parameters_high(2,:) = [1.7,0.3,0.15,0.13];

% %                   k1       k2       k3        k4                                           
%     borne_sup = [  0.25     0.4      0.15      0.1      ];
%     borne_inf = [  0.01     0.01     0.001     0.0001   ];
...
for k=1:K-1
    ...
    [tac,alpha_out,B_out(k,1,:)] = FRTM_2tissue(vect_parameters_low(k,:),vect_frame_time,M_dpa(:,k));
    if ~isempty(ind)
        alpha(:,k) = alpha_out;
    ...
    [tac,alpha_out,B_out(k,2,:)] = FRTM_2tissue(vect_parameters_high(k,:),vect_frame_time,M_dpa(:,k));
    if ~isempty(ind)
