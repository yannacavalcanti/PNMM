function [A,M,B,alpha,BP,Q,elapsedTime,f,i] = nonlin_PETunmixing(PET,A,M,B,alpha,Niter,lbd,gamma,beta,eta,alpha_lim,B_lim,epsilon,tracer_decay)
...
K = size(A,1);
% M(:,1:end-1) = M(:,1:end-1).*repmat((1+exp(-PET.time'*tracer_decay)),1,2);
M0 = M;
f = zeros(5,Niter+1);
V = size(B,3);
...
while (err > epsilon) & (i<=Niter+1)
    ...
    % Update A
    [A,W] = pnmm_A(PET,A,M,Q,B,W,eta);
    %Update M
    [ M,Q] = pnmm_M(PET,A,M,Q,E,W,M0,beta);
    
    % Objective function
    f(:,i)= objective_pnmm(PET,M,A,B,alpha,Q,W,lbd,gamma,beta,eta,M0);
