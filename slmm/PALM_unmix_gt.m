function [A,M,B,alpha,BP,Q,elapsedTime,f,i] = PALM_unmix_gt(PET,A,M,B,alpha,Niter,lbd,gamma,beta,eta,alpha_lim,B_lim,epsilon)
% PALM unmixing and variables estimation
%
%%%%%% Inputs:
...
% f(:,1) = objective_p(PET,M,A,B,alpha,Q,W,lbd,gamma,beta,eta,M0);
f(:,1) =1e10;
err = 1e10;
...
while (err > epsilon) & (i<=Niter+1)
    % Update B
    [B,W] = palm_B(PET,A,M,B,W,Q,lbd,B_lim(1,:),B_lim(2,:));

%     % Update A
    [A,W] = palm_A(PET,A,M,Q,B,W,eta);
    
    % Objective function
    f(:,i)= objective_p(PET,M,A,B,alpha,Q,W,lbd,gamma,beta,eta,M0);
    
    % Stopping criterion  or error
    err = (f(6,i-1)-f(6,i))/f(6,i-1)
    i =i+1
end
